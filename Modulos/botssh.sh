...
if ps x | grep "bot_plus"|grep -v grep 1>/dev/null 2>/dev/null; then
   clear
   echo -e "\033[1;32mO BOT HOOU_VPN JA ESTA ATIVO !\033[0m"
   sleep 3
   bot_telegram
 else
   clear
   echo -e "\033[1;32mINICIANDO BOT HOOU_VPN... \033[0m"
   sleep 3
   cd /etc/SSHPlus
   screen -dmS bot_plus bash bot > /dev/null 2>&1
   cd $HOME
   echo ""
   echo -e "\033[1;31m[\033[1;32m BOT HOOU_VPN ATIVADO \033[1;31m]\033[0m"
   sleep 3
   bot_telegram
fi
}
...
echo ""
 if ps x | grep "bot_plus"|grep -v grep 1>/dev/null 2>/dev/null; then
   clear
   echo -e "\033[1;32mPARANDO HOOU_VPN... \033[0m"
   sleep 3
   pidbot_plus=$(ps x|grep "bot_plus"|awk -F "pts" {'print $1'})
   kill -9 $pidbot_plus 1>/dev/null 2>/dev/null
   screen -wipe 1>/dev/null 2>/dev/null
   echo ""
   echo -e "\033[1;32m[ \033[1;31mBOT HOOU_VPN PARADO! \033[1;32m]\033[0m"
   sleep 3
   bot_telegram
 else
   echo ""
   echo -e "\033[1;32mO BOT HOOU_VPN JA ESTA DESATIVADO!\033[0m"
   sleep 3
   bot_telegram
 fi
}
...
reset_bot () {
	echo -e "\033[1;32mRESETANDO BOT HOOU_VPN... \033[0m"
	if ps x | grep "bot_plus"|grep -v grep 1>/dev/null 2>/dev/null; then
		pidbot_plus=$(ps x|grep "bot_plus"|awk -F "pts" {'print $1'})
	 	kill -9 $pidbot_plus 1>/dev/null 2>/dev/null
	 	screen -wipe 1>/dev/null 2>/dev/null
...
echo -e "\E[44;1;37m          BOT TELEGRAM HOOU_VPN            \E[0m"
echo ""
...
{
echo -e "\E[44;1;37m               INSTALADOR BOT HOOU_VPN                 \E[0m"
echo ""
echo -e "                 \033[1;33m● \033[1;31mATENCAO \033[1;33m●\033[0m"
echo ""
echo -e "\033[1;32m●\033[1;33m Saiba que o  BOOT HOOU_VPN ainda possui opcoes um pouco\033[0m"
...
if [[ "$resposta" = 's' ]]; then
	echo ""
  	...
  	echo ""
      clear
      echo -e "\033[1;32mINICIANDO  BOOT HOOU_VPN \033[0m"
      echo ""
      sleep 1
  	token2=$(cat /etc/SSHPlus/bot|grep "api_bot=" | awk -F = '{print $2}')
  	sed -i "s/api_bot=$token2/api_bot='$tokenbot'/" /etc/SSHPlus/bot
  	...
  	cd $HOME
      echo ""
      echo -e "\033[1;31m[\033[1;32m  BOOT HOOU_VPN ATIVADO \033[1;31m]\033[0m"
      sleep 2
      clear
  	bot_telegram
  else
